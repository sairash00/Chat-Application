import { useState } from "react";
import { FaRegEye } from "react-icons/fa";
import { FaRegEyeSlash } from "react-icons/fa6";
import { useNavigate } from "react-router-dom";
import { RegisterData } from "../../Interfaces/interface";
import { useMutation } from "@tanstack/react-query";
import { sendOtp } from "../../Functions/QueryFns";
import showToast from "../../Functions/CustomToast";
import {handleRegistrationError } from "../../Functions/FetchErrorHandler";
import { verifyRegisterData } from "../../Functions/DataCheck";
import { PulseLoader } from "react-spinners";

const RightSignUpComponent = () => {
  const navigate = useNavigate()  
  const [showPassword, setShowPassword] = useState(false)
  const [registerData, setRegisterData] = useState<RegisterData>({
    email:"",
    password: "",
    username: ""
  })

  const {mutate, status} = useMutation({
    mutationFn: sendOtp,
    onSuccess: (data) => {
      showToast("OTP sent Successfully", "success")
      setRegisterData({
        email: "",
        password: "",
        username: ""
      })
      navigate("/verification/otp", {state: data.data?.user} )
    },
    onError: (error) => {
      const errorMessage = handleRegistrationError(error)
      showToast(errorMessage, "error")
      setRegisterData({
        ...registerData,
        password: ""
      })
    }
  })

  const handleShowPassword = () => {
    setShowPassword(!showPassword)
  }

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value, name } = e.target as HTMLInputElement
    setRegisterData({
      ...registerData,
      [name]: value
    })
  }

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const verifiedRegisterData:RegisterData | string = verifyRegisterData(registerData)
    if (typeof verifiedRegisterData === "string") {
      showToast(verifiedRegisterData, "error")
      return
    }
    mutate(verifiedRegisterData)
  }


  return (
    <div className="flex flex-col shadow-inner  justify-center items-center w-full md:w-1/2 p-8 backdrop-blur-md rounded-none md:rounded-bl-none md:rounded-tr-3xl">
        <h2 className="text-3xl font-semibold mb-6 text-[#212121]  ">Sign Up</h2>
        <form onSubmit={handleSubmit} className="w-full max-w-sm">
          <div className="mb-4">
            <label className="block text-[#212121] text-sm font-bold tracking-wide" htmlFor="email">
              E-mail
            </label>
            <input
              className="w-full py-2 px-1 text-[#212121] border-b-2 border-blue-400 focus:outline-none focus:border-[#577fee] bg-transparent font-semibold tracking-wider"
              id="email"
              name="email"
              onChange={handleChange}
              value={registerData.email}
              type="email"
            />
          </div>
          <div className="mb-4">
            <label className="block text-[#212121] text-sm font-bold tracking-wide " htmlFor="username">
              Username
            </label>
            <input
              className="w-full py-2 px-1 text-[#212121] border-b-2 border-blue-400 focus:outline-none focus:border-[#577fee] bg-transparent font-semibold tracking-wider"
              id="username"
              name="username"
              onChange={handleChange}
              value={registerData.username}
              type="text"
            />
          </div>
          <div className="mb-6">
            <label className="block text-[#212121] text-sm font-bold tracking-wide " htmlFor="password">
              Password
            </label>
            <div className="relative">
              <input
                className="w-full py-2 px-1 text-[#212121]  border-b-2 border-blue-400 focus:outline-none focus:border-[#577fee] bg-transparent font-semibold tracking-wider"
                id="password"
                name="password"
                onChange={handleChange}
                value={registerData.password}
                type={showPassword ? "text":"password"}
              />
              <span onClick={handleShowPassword} className="absolute inset-y-0 right-0 mr-3 flex items-center text-xl text-[#212121] ">
                {showPassword? <FaRegEyeSlash/> : <FaRegEye/>}
              </span>
            </div>
          </div>
          <button
            className="w-full bg-[#6179F9] text-[#fff] font-bold py-2 px-4 rounded-full hover:bg-[#596bd1cb] focus:outline-none focus:shadow-outline transition"
            type="submit"
            disabled = {status === 'pending' ? true : false}
          >
            { status === 'pending' ? <PulseLoader size={10} color="#D7E7FD" /> : "Sign Up"}
          </button>
        </form>
      </div>
  )
}

export default RightSignUpComponent